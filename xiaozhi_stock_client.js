...
function connect() {
  ...
  ws.on('open', () => { 
    ...
    // 主动心跳：30s 发 ws 协议级 ping（RFC6455，对端 ws 库必须回 pong，与 MCP 协议无关）
    // 若一个周期没收到 pong → 连接半死，强制 terminate 触发重连
    ws._hbAlive = true;
    ws.on('pong', () => { ws._hbAlive = true; });
    ws._hb = setInterval(() => {
      try {
        if (ws.readyState !== WebSocket.OPEN) return;
        if (ws._hbAlive === false) { console.log('[' + new Date().toISOString() + '] Heartbeat timeout, force reconnect'); try { ws.terminate(); } catch (e) {} return; }
        ws._hbAlive = false;
        ws.ping();
      } catch (e) {}
    }, 30000);
